...
export function InternshipsForm({ form, isEdit }: Readonly<Props>) {
        ...
        return (
          <div key={field.id} className="mb-4 space-y-4 rounded-lg border-2 border-dashed p-4 ">
            ...
            <div className="grid grid-cols-12 gap-4 p-2">
              <FormField
                name={`internships.${index}.name`}
                control={form.control}
                render={({ field }) => (
                  <FormItem className="col-span-12 lg:col-span-9">
                    <FormLabel className="font-bold">Nome *</FormLabel>
                    <FormControl>
                      <Input {...field} disabled={isEdit} placeholder="Ex: Estágio obrigatório" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                name={`internships.${index}.hoursWorkload`}
                control={form.control}
                render={({ field }) => (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="col-span-12">
                    <FormLabel className="font-bold">Descrição do Estágio *</FormLabel>
                    <FormControl>
                      <Textarea
                        {...field}
                        disabled={isEdit}
