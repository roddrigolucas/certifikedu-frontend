...
import { CheckIcon, Clock9Icon } from 'lucide-react';
...
export function Footer() {
  const { profileData, isUserEnabled } = useProfile();
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2">
        <Link to={pagePaths.authenticated.account.profile} className="w-full">
          <Button
            ...
            variant="ghost"
            className="inline-flex h-fit w-full justify-start gap-3 truncate px-3 py-4 text-left hover:bg-slate-800/60"
          >
            <Avatar className="relative overflow-visible">
              ...
              <AvatarImage src="123" />
              <AvatarFallback>{getInitials(profileData?.naturalPerson.name ?? '')}</AvatarFallback>
            </Avatar>
