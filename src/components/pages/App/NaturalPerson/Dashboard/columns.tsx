...
import {
  IAbilityCertificate,
  ICertificate,
} from '@/services/entities/app/legalPerson/certificates/model';
...
export const columns: ColumnDef<ICertificate>[] = [
  ...
  {
    accessorKey: 'certificateReceptor',
    header: ({ column }) => <TableColumnHeader column={column} title="Emitido para" />,
    enableSorting: true,
    enableHiding: false,
    cell: ({ row }) => (
      <div className="inline-flex w-full items-center gap-2">
        ...
        <span className="font-semibold">{row.getValue('certificateReceptor')}</span>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      switch (row.getValue('successStatus')) {
        case 'PENDING':
